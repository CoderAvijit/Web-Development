...
let length = document.getElementsByClassName("product").length+1;
var list = new Array();
addProductBtn.addEventListener('click', function(event) {
  ...
  reader.addEventListener('load', function() {
    const product = createProduct(reader.result, productName,productDetails, productPrice);
...
function createProduct(imageUrl, name, details, price) {
  const div = document.createElement('div');
  div.className = 'product';
  div.setAttribute('id',"product-"+length);
